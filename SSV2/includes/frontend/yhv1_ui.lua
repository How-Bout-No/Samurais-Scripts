...
local function drawBasicTab()
	---@type HEIST_TYPES
	local HEIST_TYPES = {
		{ name = "Cluckin Bell", location = vec3:new(-1093.15, -807.14, 20.28), blip = 871, stat = "MPX_SALV23_INST_PROG",  val = 31 },
		{ name = "KnoWay", location = vec3:new(42.82, -1599.19, 30.60), blip = 76, stat = "MPX_M25_AVI_MISSION_CURRENT",  val = 4 },
		{ name = "Dr. Dre", location = YHV1:GetAgencyLocation(), blip = 826, stat = "MPX_FIXER_STORY_BS",  val = 4095  },
		{ name = "Oscar Guzman", location = vec3:new(2150.65, 4796.60, 42.17), blip = 903, stat = "MPX_HACKER24_INST_BS", val = 31, optInfo = "Complete first mission on Hard first!" },
	}
...
local cayo_secondary_target_i = 0
local cayo_secondary_target_c = 0
...
local function drawCayoTab()
	...
	local cayo_heist_approach        = stats.get_int("MPX_H4_MISSIONS")
	local cayo_heist_weapons         = stats.get_int("MPX_H4CNF_WEAPONS")
	local cayo_heist_disruptions     = stats.get_int("MPX_H4CNF_WEP_DISRP") + stats.get_int("MPX_H4CNF_ARM_DISRP") + stats.get_int("MPX_H4CNF_HEL_DISRP")
	...
	if GUI:Button("Set Secondary Targets") then
	...
	local new_approach, approach_clicked = ImGui.Combo(
		_T "YH_CAYO_APPROACH",
		cayo_heist_approach,
		{ "Kosatka", "Alkonost", "Velum", "Stealth Annihilator", "Patrol Boat", "Longfin", "All Vehicles" },
		7
	)

	if (approach_clicked) then
		local val = Switch(new_approach) {
			[0] = 65283,
			[1] = 65413,
			[2] = 65289,
			[3] = 65425,
		...
		stats.set_int("MPX_H4_MISSIONS", val)
	end
	...
	ImGui.BeginDisabled(cayo_heist_disruptions == 9)
	if GUI:Button(_T "YH_CAYO_DISRUPTION") then
